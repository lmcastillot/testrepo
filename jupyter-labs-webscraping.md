...
response=requests.get(static_url, headers={'User-Agent': 'Chrome/91.0.4472.124 Safari/537.36'})
...
```python
column_names = []
tem = soup.find_all('th')
for tag in range(len(tem)):
    try:
        x = extract_column_from_header(tem[tag])
...
```
    ...
    /tmp/ipykernel_515/354354048.py in <module>
         60             # Customer
...
```
    ...
    Empty DataFrame
    Columns: [Flight No., Launch site, Payload, Payload mass, Orbit, Customer, Launch outcome, N/A, FH 2, FH 3, Demonstrations, logistics, Crewed, Commercial satellites, Scientific satellites, Military satellites, Rideshares, Transporter, Bandwagon, Flight tests, Current, In development, Retired, Cancelled, Spacecraft, Cargo, Test vehicles, Landing sites, Other facilities, Support, Contracts, R&D programs, Key people, Related, General, People, Vehicles, Launches by rocket type, Launches by spaceport, Agencies, companies and facilities, Other mission lists and timelines, Version Booster, Booster landing, Date, Time]
    Index: []
